...
DuAnChiTietControl = {
    Init: function () {
        DuAnChiTietControl.RegisterEventsGiayPhepMoiTruong();
    },
                        ...
                        $.each(value.FileTaiLieu, function (j, file) {
                            switch (file.LoaiFileTaiLieu) {
                                case "GiayPhepMoiTruong":
                                    ...
                                    html1 += '<span class=""><a href = "' + localStorage.getItem('API_URL').replace("api", "") + file.File.LinkFile + '">' + iconfile + file.File.TenFile + '</a></span>';
                                    break;
                                case ("DonXinCapPhep"):
                                    var iconfile = self.DrawIconFile(file.File.LinkFile);
                                    html2 += '<span class=""><a href = "' + localStorage.getItem('API_URL').replace("api", "") + file.File.LinkFile + '">' + iconfile + file.File.TenFile + '</a></span>';
                                    break;
                                case ("BaoCaoDeXuatCapPhep"):
                                    var iconfile = self.DrawIconFile(file.File.LinkFile);
                                    html3 += '<span class=""><a href = "' + localStorage.getItem('API_URL').replace("api", "") + file.File.LinkFile + '">' + iconfile + file.File.TenFile + '</a></span>';
                                    break;
                                case ("DuAnDauTu"):
                                    var iconfile = self.DrawIconFile(file.File.LinkFile);
                                    html4 += '<span class=""><a href = "' + localStorage.getItem('API_URL').replace("api", "") + file.File.LinkFile + '">' + iconfile + file.File.TenFile + '</a></span>';
                                    break;
                                default:
                                    var iconfile = self.DrawIconFile(file.File.LinkFile);
                                    html5 += '<span class=""><a href = "' + localStorage.getItem('API_URL').replace("api", "") + file.File.LinkFile + '">' + iconfile + file.File.TenFile + '</a></span>';
                            }
    ...
    RegisterEventsGiayPhepMoiTruong: function () {
        var self = this;
        var $popup = $('#popup-form-giay-phep-moi-truong');
        $('#btnCreateGiayPhepMoiTruong').off('click').on('click', function () {
            $popup.modal('show');
            self.ResetPopup();
            self.RegisterEventsPopupGiayPhepMoiTruong();
        });
